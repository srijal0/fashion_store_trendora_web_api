...
function FallbackAvatar({ name }: { name: string }) {
  ...
  return (
    <div className="w-full h-full rounded-full bg-pink-200 flex items-center justify-center">
      <span className="text-pink-600 font-bold text-2xl">{initials}</span>
    </div>
  );
}
...
export default function ProfileForm({ userId, initialData }: ProfileFormProps) {
  ...
  return (
    <form
      onSubmit={handleSubmit(onSubmit)}
      className="space-y-6 p-6 bg-white rounded-lg shadow-md max-w-md mx-auto"
    >
      ...
      <div className="flex flex-col items-center">
        <div className="w-24 h-24 rounded-full overflow-hidden border">
          {imagePreview ? (
            ...
            <FallbackAvatar name={initialData?.name || "User"} />
          )}
        </div>
